...
class SmoothClassifier(nn.Module):
    ...
    def certify(self, inputs: torch.Tensor, n0: int, num_samples: int, alpha: float, batch_size: int) -> Tuple[int,
        ...
        # Then, using a large set of samples we compute the lower bound
        class_counts_lower_bound = self._sample_noise_predictions(inputs, num_samples, batch_size)
        num_top_class = class_counts_lower_bound[top_class].cpu()
    ...
    def _sample_noise_predictions(self, inputs: torch.tensor, num_samples: int, batch_size: int) -> torch.Tensor:
        ...
        with torch.no_grad():
            ...
            for it in range(ceil(num_samples / batch_size)):
                ...
                guassian_noise = torch.randn_like(batch) * self.sigma# [this_batch_size, C, N, N]

                class_prediction_on_this_batch = self.base_classifier((batch+guassian_noise).clamp(0,1)).argmax(-1,keepdim=True) # [this_batch_size,1]

                class_counts += (class_prediction_on_this_batch == classes).long().sum()
